...
#         flow_data = f2.get('FTLE_back_0_6s_finegrid')[tmin-30:tmax-30, xmin*2:xmax*2, ymin*2:ymax*2]
...
#     odor_data = np.flip(odor_data, axis=1)
...
#     # FTLE_x = np.linspace(x_grid[0, 0], x_grid[0, -1], len(flow_data[0, :])-1)
#     # FTLE_y = np.linspace(y_grid[0, 0], y_grid[-1, 0], len(flow_data[:, 0])-1)
...
#     # vmin = -2
#     # vmax = 8
#     # plt.contourf(FTLE_x, FTLE_y, flow_data[:-1, :-1], 100, vmin=vmin, vmax=vmax, cmap=colormap, alpha=1)
#     # plt.colorbar()
...
#     # vmin = 0.0005
#     # vmax = 1
#     # plt.pcolormesh(x_grid, y_grid, np.flipud(odor_data[:-1, :-1]), cmap=colormap, norm=colors.LogNorm(vmin=vmin, vmax=vmax), alpha=0.3)
#     # plt.colorbar()
...
#         flow_ts = pd.Series(flow_data[:, y*2, x*2])
#         # print(flow_ts.size)
...
#     stats_df.to_pickle(f'ignore/data/meanEratio_x{round(xmin*dx, 3)}to{round(xmax*dx, 3)}_y{round(ymin*dx-0.3, 3)}to{round(ymax*dx-0.3, 3)}_t{round(tmin*dt, 0)}to{round(tmax*dt, 0)}_wthr{whiff_threshold}.pkl')
    ...
    stats_df = pd.read_pickle('ignore/data/meanEratio_x0.15to0.251_y-0.15to0.151_t1.0to10.0_wthr0.01.pkl')

    # PLOTTING
    plt.close()
    vmin = 0
    vmax = 10
    plt.scatter(stats_df['y'], stats_df['x'], c=stats_df['mean_E_ratio'], vmin=vmin, vmax=vmax)
    plt.colorbar()
